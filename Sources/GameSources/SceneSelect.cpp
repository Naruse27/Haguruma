...
// 初期化
void SceneSelect::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
    this->device = device;
    ...
    selectSprite = new Sprite(device, L"./Data/Sprite/select.png");
...
void SceneSelect::Update(float elapsedTime)
{
    ...
    //if (mouse.GetButtonDown() & Mouse::BTN_LEFT) SceneManager::Instance().ChangeScene(new SceneTitle);
    if (screenPosition.x > 1050 && screenPosition.x < 1150 &&
        screenPosition.y > 340  && screenPosition.y < 440)
    {
        if (mouse.GetButtonDown() & Mouse::BTN_LEFT) SceneManager::Instance().ChangeScene(new SceneSelect2);
    }
