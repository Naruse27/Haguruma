...
// 初期化
void DebugRenderer::Init(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
...
void DebugRenderer::Render(ID3D11DeviceContext* context, const DirectX::XMFLOAT4X4& view, const DirectX::XMFLOAT4X4& projection)
{
	...
	// myAdd
	context->IASetVertexBuffers(0, 1, stageMeshVertexBufferMain.GetAddressOf(), &stride, &offset);
	if (stageMeshDraw) {
		DirectX::XMMATRIX W = DirectX::XMMatrixIdentity();
		DirectX::XMMATRIX WVP = W * VP;
		// 定数バッファ更新
		CbMesh cbMesh;
		cbMesh.color = { 1,0,1,1 };
		DirectX::XMStoreFloat4x4(&cbMesh.wvp, WVP);

		context->UpdateSubresource(constantBuffer.Get(), 0, 0, &cbMesh, 0, 0);
		context->Draw(stageMeshVertexCount, 0);
	}

	context->IASetVertexBuffers(0, 1, stageMeshVertexBufferMove.GetAddressOf(), &stride, &offset);
	if (stageMeshDrawMove) {
		DirectX::XMMATRIX W = DirectX::XMMatrixIdentity();
		DirectX::XMMATRIX WVP = W * VP;
		// 定数バッファ更新
		CbMesh cbMesh;
		cbMesh.color = { 1,1,0,1 };
		DirectX::XMStoreFloat4x4(&cbMesh.wvp, WVP);

		context->UpdateSubresource(constantBuffer.Get(), 0, 0, &cbMesh, 0, 0);
		context->Draw(stageMeshVertexCountMove, 0);
	}
}
...
void DebugRenderer::CreateStageMainMesh(ID3D11Device* device, std::vector<DirectX::XMFLOAT3>& vertices)
{
	if (stageMeshVertexBufferMain.Get() != nullptr) stageMeshVertexBufferMain->Release();
	stageMeshDraw = true;
	stageMeshVertexCount = vertices.size();
	// 頂点バッファ
	{
		D3D11_BUFFER_DESC desc{};
		D3D11_SUBRESOURCE_DATA subresourceData{};
		desc.ByteWidth = static_cast<UINT>(sizeof(DirectX::XMFLOAT3) * stageMeshVertexCount);
		desc.Usage = D3D11_USAGE_IMMUTABLE; //D3D11_USAGE_DEFAULT
		desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		desc.CPUAccessFlags = 0;
		desc.StructureByteStride = 0;
		...
		HRESULT hr = device->CreateBuffer(&desc, &subresourceData, stageMeshVertexBufferMain.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	}
}

void DebugRenderer::CreateStageMoveMesh(ID3D11Device* device, std::vector<DirectX::XMFLOAT3>& vertices)
{
	if (stageMeshVertexBufferMove.Get() != nullptr) stageMeshVertexBufferMove->Release();
	stageMeshDrawMove = true;
	stageMeshVertexCountMove = vertices.size();
	// 頂点バッファ
	{
		D3D11_BUFFER_DESC desc{};
		D3D11_SUBRESOURCE_DATA subresourceData{};
		desc.ByteWidth = static_cast<UINT>(sizeof(DirectX::XMFLOAT3) * stageMeshVertexCountMove);
		desc.Usage = D3D11_USAGE_IMMUTABLE; //D3D11_USAGE_DEFAULT
		desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		desc.CPUAccessFlags = 0;
		desc.StructureByteStride = 0;
		...
		HRESULT hr = device->CreateBuffer(&desc, &subresourceData, stageMeshVertexBufferMove.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	}
}