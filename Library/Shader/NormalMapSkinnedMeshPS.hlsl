...
{
#if 1
    float3 E = normalize(EyePosition.xyz - pin.worldPos);
	...
	*/
	...
	// テクスチャ色取得
    float4 color = diffuseTexture.Sample(anisotropicSampler, pin.texcoord);
    float3 N = normalTexture.Sample(pointSampler, Texcoord).xyz;
    N = N * 2.0f - 1.0f;
